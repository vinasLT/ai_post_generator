    ...
    async def get_calculator_for_lot(cls, lot: lot_pb2.Lot) -> calculator_pb2.GetCalculatorWithDataResponse | None:
        async with CalculatorRcpClient() as client:
            try:
                ...
                return None
    @classmethod
    async def get_calculators_for_lots(cls, lots: list[lot_pb2.Lot]) -> list[tuple[lot_pb2.Lot, calculator_pb2.GetCalculatorWithDataResponse]]:
        lots_with_calculators = []

        async def get_calculator(lot: lot_pb2.Lot) -> None:
            try:
                calculator = await cls.get_calculator_for_lot(lot)
                if calculator:
    ...
    async def create_posts_batch(
        ...
        async with get_async_db() as db:
            for lot_calculator_pair in lots_with_calculators:
                ...
                try:
                    post = await cls.create_post(lot, calculator, request_id, db, flush=True)
                    if post:
